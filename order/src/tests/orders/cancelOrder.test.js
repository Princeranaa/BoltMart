...
describe('POST /api/orders/:id/cancel — Buyer-initiated cancel while rules apply', () => {
    const orderId = '6931518e57d7e60e00a63535';
    ...
    it('cancels a PENDING order and returns updated order', async () => {
        ...
        const order = new orderModel({
            _id: orderId,
            user: '692d94f3e4521fd8b8ed3ceb', // ensure this matches auth user
            status: 'PENDING',
            items: [
                {
                    product: '692ee3d0adfa4ebccd655aae',
                    quantity: 1,
    ...
    it('returns 403 when user is not the owner of the order', async () => {
        ...
        const res = await request(app)
            .post(`/api/orders/${orderId}/cancel`)
            .set('Cookie', getAuthCookie({ userId: '123456789012345678901234' }))
            .expect('Content-Type', /json/)
            .expect(403);
